...
image:media/ros_node_topic_overview.png[image]
...
|App_publish_0|Fetches JSON data from the App supplier Radeffect and publishes data on ROS topics and in the PostgreSQL database
||Skylab process
|Speech_0|Subscribes to the conversation topic, determines how positive/negative the text is through Azure and publishes this on a ROS topic
|App_0	|To be decided if necessary
||Skylab output
|Skylab_topic_listen_0|Fetches data from ROS topics and publishes data in the PostgreSQL database
|App_publish_0|Fetches data from ROS topics and publishes JSON data to the App supplier Radeffect and in the PostgreSQL database
|===============================================
